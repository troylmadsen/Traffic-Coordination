...
/**
 * A simulator with only autonomous drivers and forward-facing snesors to track
    ...
    for(VehicleSimView vehicle: vinToVehicles.values()) {
      ...
      if (vehicle instanceof AutoVehicleSimView) {
        ...
        if (autoVehicle.isVehicleTracking()) {
          ...
          Map<Lane, SortedMap<Double, VehicleSimView>> vehiclesOnComingLane =
        		  new HashMap<Lane, SortedMap<Double, VehicleSimView>>();
          for (Entry<Lane, SortedMap<Double, VehicleSimView>> e:
        		  vehicleLists.entrySet()) {
        	  if (e.getKey() != targetLane) {
        		  vehiclesOnComingLane.put(e.getKey(),
        				  new TreeMap<Double, VehicleSimView>(e.getValue()));
        	  }
          }
          ...
          /* Troy Madsen */
          double vehicleX = vehicle.getCenterPoint().getX();
          double vehicleY = vehicle.getCenterPoint().getY();
          Line2D rightSensor= new Line2D.Double(vehicleX,
        		  vehicleY,
        		  vehicleX + Math.cos(45) * sensorDistance,
        		  vehicleY + Math.sin(45) * sensorDistance);
          if (rightSensor.intersects((Rectangle2D)vehicle.getShape())) {
        	  System.out.println("Right detection!");
          }
          Line2D leftSensor= new Line2D.Double(vehicleX,
        		  vehicleY,
        		  vehicleX + Math.cos(315) * sensorDistance,
        		  vehicleY + Math.sin(315) * sensorDistance);
          if (leftSensor.intersects((Rectangle2D)vehicle.getShape())) {
        	  System.out.println("Left detection!");
          }
